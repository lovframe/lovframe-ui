...
function simple_button:create(type, mode, x, y, width, height, r, g, b, h_r, h_g, h_b, button)
    self.type = type
    self.mode = mode
    self.x = x
    self.y = y
...
function simple_button:onpressed(button)
    if button == 1 and mouse_x > self.x and mouse_x < self.x + self.width then
        if mouse_y > self.y and mouse_y < self.y + self.height then
            return true
        end
    else
